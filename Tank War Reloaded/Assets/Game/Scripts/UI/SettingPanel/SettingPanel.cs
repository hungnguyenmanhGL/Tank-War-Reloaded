...
public class SettingPanel : UIFrame {
    [Header("[References]")]
    ...
    private void RestorePurchaseCompleted(string[] products) {
        ...
        UIManager.Instance.Push<DialogPanel>().Dialog("Restore purchase completed!", message);
    }

    private void RestorePurchaseFailed(string error) {
        UIManager.Instance.Push<DialogPanel>().Dialog("Restore purchase failed!", error);
    }
}
